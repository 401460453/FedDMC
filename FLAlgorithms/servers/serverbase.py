...
class Server:
    ...
    def get_param_weight(self, user, fea_sum, pre_sum):
        user.model.eval()
        user.generative_model.eval()
        samples = user.get_next_train_batch(count_labels=True)
        X, y = samples['X'], samples['y']
        real_features = user.model(X, logit=True, ke=user.ke)['feature']
        real_feature = torch.mean(real_features, dim=0, keepdim=True)
        gen_features = user.generative_model(y, latent_layer_idx=user.latent_layer_idx)['output']
        gen_feature = torch.mean(gen_features, dim=0, keepdim=True)
        pdist = nn.PairwiseDistance(p=2)
        distance = pdist(real_feature, gen_feature)
        fea_sum += 1/distance
        pre_sum += user.train_samples
        return [1/distance, user.train_samples], fea_sum, pre_sum

    def hierarchical_add_parameters(self, user, weight):

        for (server_name, server_param),  (user_name, user_param) in zip(self.model.named_parameters(), user.model.named_parameters()):
            if server_name.find('8') >= 0:
                server_param.data = server_param.data + user_param.data.clone() * weight[user.id][1]
            else:
                server_param.data = server_param.data + user_param.data.clone() * weight[user.id][0]

    def hierarchical_aggregate_parameters(self):
        assert (self.selected_users is not None and len(self.selected_users) > 0)

        user_param_weight = {}
        feature_weight_sum = 0
        predict_weight_sum = 0
        for user in self.selected_users:
            user_param_weight[user.id], feature_weight_sum, predict_weight_sum = self.get_param_weight(user, feature_weight_sum, predict_weight_sum)

        for user in self.selected_users:
            user_param_weight[user.id][0] = user_param_weight[user.id][0] / feature_weight_sum
            user_param_weight[user.id][1] = user_param_weight[user.id][1] / predict_weight_sum
        ...
        for user in self.selected_users:
            total_train += user.train_samples
        for user in self.selected_users:
            self.hierarchical_add_parameters(user, user_param_weight)

    def aggregate_fisher(self):
        assert (self.selected_users is not None and len(self.selected_users) > 0)
        for n, p in self.model.named_parameters():
            self.globe_fisher[n] = 0 * p.data

        for user in self.selected_users:
            for n, _ in user.model.named_parameters():
                self.globe_fisher[n] += (user.local_fisher[n]) / self.num_users
    ...
    def test(self, selected=False, ke=None):
    ...
    def evaluate(self, save=True, selected=False, ke=None):
        # override evaluate function to log vae-loss.
        test_ids, test_samples, test_accs, test_losses = self.test(selected=selected, ke=ke)
