...
def get_data_dir(dataset, env):
    if 'EMnist' in dataset:
        ...
        path_prefix=os.path.join('data', 'Mnist', 'u20c10-alpha{}-ratio{}-env{}'.format(alpha, ratio, env))
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
...
def read_data(dataset, env):
    ...
    '''
    train_data_dir, test_data_dir, proxy_data_dir = get_data_dir(dataset, env)
    clients = []
    groups = []
    train_data = {}
    test_data = {}
...
def create_generative_model(dataset, algorithm='', model='cnn', embedding=False):
    ...
    assert any([alg in algorithm for alg in ['FedGen', 'FedGen', 'FedDMC']])
